...
import { ClipboardList, CheckCircle, XCircle, Truck, PackageCheck, PackageSearch, DollarSign } from 'lucide-react';
...
const DriverView = () => {
  ...
  const handleAction = (shipmentId, action, newPrice = null) => {
    ...
    switch (action) {
      case 'accept_original':
        statusUpdate = { status: 'ACCEPTED_BY_DRIVER', finalPrice: selectedShipment.originalPrice };
        toastMessage = "Oferta aceptada con precio original.";
        break;
      case 'deny':
        statusUpdate = { status: 'DENIED_BY_DRIVER' };
      ...
      case 'suggest_price':
        ...
        statusUpdate = { status: 'PRICE_SUGGESTED_BY_DRIVER', driverSuggestedPrice: parseFloat(newPrice) };
  ...
  const myActiveShipments = allShipments.filter(s => ['ACCEPTED_BY_DRIVER', 'ACCEPTED_BY_USER', 'PICKED_UP'].includes(s.status));
          ...
          {pendingRequests.length === 0 ? (
            ...
            <div className="space-y-4">
              {pendingRequests.map((shipment) => (
                <motion.div key={shipment.id} variants={itemVariants} initial="hidden" animate="visible" transition={{ delay: 0.1 * pendingRequests.indexOf(shipment) }}>
          ...
          {myActiveShipments.length === 0 ? (
            ...
            <div className="space-y-4">
              {myActiveShipments.map((shipment) => (
                <motion.div key={shipment.id} variants={itemVariants} initial="hidden" animate="visible" transition={{ delay: 0.1 * myActiveShipments.indexOf(shipment) }}>
                  <Card className="bg-slate-800/70 border-slate-700">
                    <CardHeader>
